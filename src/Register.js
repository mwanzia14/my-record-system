...
/* Panel themes (mirrored from Login)  */
const PANEL_THEMES = [
  {
    bg:         '#050a14',
    grid:       'rgba(0,200,255,.055)',
    orb1:       'radial-gradient(circle,rgba(0,80,255,.36),transparent)',
    orb2:       'radial-gradient(circle,rgba(0,200,255,.2),transparent)',
    orb3:       'radial-gradient(circle,rgba(120,0,255,.2),transparent)',
    accent:     '#00c8ff',
    accentRgb:  '0,200,255',
    accent2:    '#7c6fff',
    eyebrow:    'rgba(0,200,255,.6)',
    eyebrowText:'Your setup takes 60 seconds',
    heroSpan:   '#00c8ff',
...
const GlobalStyle = createGlobalStyle`
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .orb-b { animation-duration:9s;  animation-delay:2.5s; }
  .orb-c { animation-duration:16s; animation-delay:4s; }
...
const Shell = styled.div`
  ...
  @media(max-width:768px){ grid-template-columns:1fr; & > div:last-child { display:none; } }
`;
...
/* Centered logo row — no brand name text */
...
const FormSub= styled.p`
  font-size:.87rem; font-weight:300; color:#7a7a9a;
  margin-bottom:2rem; text-align:center;
`;
const FGroup = styled.div`display:flex; flex-direction:column; gap:.9rem; margin-bottom:1.4rem;`;
const FL     = styled.label`
  font-size:.7rem; font-weight:700; letter-spacing:.1em; text-transform:uppercase;
  color:#3a3a5a; margin-bottom:.3rem; display:block;
`;
...
`;
const PwRow = styled.div`display:flex; gap:3px; margin-top:.4rem;`;
const PwBar = styled.div`
  flex:1; height:3px; border-radius:3px;
  background:${p => p.$on ? (p.$c || '#1a1aff') : '#e4e4f0'}; transition:background .3s;
`;
const PwLbl = styled.span`
  font-size:.67rem; font-weight:600; color:${p => p.$c || '#bbb'}; display:block; margin-top:.25rem;
`;
    ...
    <AnimatePresence mode="wait">
      ...
      >
        <svg viewBox="0 0 460 300" fill="none" xmlns="http://www.w3.org/2000/svg"
          ...
          <ellipse cx="80"  cy="64"  rx="60" ry="60" fill={`url(#nodeGlowC_${themeIdx})`}/>
          <ellipse cx="240" cy="156" rx="60" ry="60" fill={`url(#nodeGlowC_${themeIdx})`}/>
          <ellipse cx="390" cy="240" rx="60" ry="60" fill={`url(#nodeGlowC_${themeIdx})`}/>
          ...
          <text x="80" y="74" textAnchor="middle" fill="rgba(255,255,255,0.35)" fontSize="7.5" fontFamily="sans-serif">Create</text>
          ...
          <text x="240" y="166" textAnchor="middle" fill="rgba(255,255,255,0.35)" fontSize="7.5" fontFamily="sans-serif">Set up</text>

          {/* Step 2 card */}
          <rect x="282" y="122" width="152" height="68" rx="9"
            fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.07)" strokeWidth="1"/>
          <text x="294" y="140" fill="rgba(255,255,255,0.3)" fontSize="7.5" fontFamily="sans-serif">Project type</text>
          {[['Word Count', accent, true],['CPP Rate','rgba(255,255,255,0.12)',false],['Flat Rate','rgba(255,255,255,0.12)',false]].map(([t,bg,active],i) => (
            <g key={i}>
          ...
          <text x="294" y="176" fill="rgba(255,255,255,0.3)" fontSize="7.5" fontFamily="sans-serif">Rate per word</text>
          <rect x="294" y="181" width="130" height="5" rx="2.5" fill="rgba(255,255,255,0.08)"/>
          <rect x="294" y="181" width="70"  height="5" rx="2.5" fill={`rgba(${accentRgb},0.5)`}/>
          ...
          <text x="390" y="250" textAnchor="middle" fill="rgba(255,255,255,0.35)" fontSize="7.5" fontFamily="sans-serif">Export</text>
...
function getPwStrength(pw) {
  ...
  if (strong && pw.length >= 8) return { level:3, label:'Strong', color:'#16b364' };
    ...
    <>
      ...
      <Shell>
        ...
        <LeftPanel>
          ...
          >
            {/* Logo centered above heading — no brand name text */}
            <BrandRow>
              <LogoImg src={logo} alt="Logo" />
            </BrandRow>

            <FormH>Create your account</FormH>
            <FormSub>Join and start tracking orders in under a minute.</FormSub>
                ...
                <div>
                  ...
                  {password.length > 0 && (
                    <>
                      <PwRow>
                        {[1,2,3].map(n => (
                          <PwBar key={n} $on={strength.level >= n} $c={strength.color}/>
                        ))}
                      </PwRow>
                      <PwLbl $c={strength.color}>{strength.label}</PwLbl>
                    </>
                  )}
                </div>
                ...
                <div>
                  ...
                  <FW>
                    ...
                    <Input id="confirmPassword" type="password" placeholder="Re-enter your password"
                      value={confirmPassword} $f={focus==='confirm'}
            ...
            <Divider>or</Divider>
            <Foot>Already have an account? <a href="/login">Sign in</a></Foot>
          </motion.div>
        ...
        <RightPanel $bg={pt.bg}>
          <GridBg $grid={pt.grid} />
          <Orb className="orb orb-a" style={{ width:360, height:360, background:pt.orb1, top:'-8%', right:'-10%' }}/>
          <Orb className="orb orb-b" style={{ width:200, height:200, background:pt.orb2, bottom:'18%', left:'6%' }}/>
          <Orb className="orb orb-c" style={{ width:130, height:130, background:pt.orb3, top:'40%', right:'18%' }}/>
          ...
          >
            ...
            <HeroH>
              Stop chasing.<br/>
              <span style={{ color: pt.heroSpan, transition: 'color 1.8s ease' }}>
                Start collecting.
              </span>
            </HeroH>
            <HeroP>Create an account, set your rates, export your first invoice. Clean books from day one — no chasing required.</HeroP>
          </TopText>
